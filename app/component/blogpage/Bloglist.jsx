...
import { formatDate, postPath, readingTime } from "../../utils/blog";
...
const CoverImage = ({ post, className }) =>
  post.cover ? (
...
const PostCard = ({ post }) => (
  <motion.article variants={fadeUp} className="h-full">
    <Link to={postPath(post)} className="group flex h-full flex-col">
    ...
    <>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-16 sm:px-6 md:pb-24">
        <div className="flex flex-wrap justify-center gap-2.5 md:gap-3">
          <Link
            to={hrefWith({ category: null, page: null })}
            preventScrollReset
          ...
          {categories.map((cat) => {
            ...
            return (
              <Link
                key={cat.slug}
                to={hrefWith({ category: cat.slug, page: null })}
                preventScrollReset
                className={`rounded-full border px-4 py-2 text-sm font-semibold transition duration-300 ${
                  active
                    ? "border-[#0D99FF] bg-[#0D99FF] text-white shadow-md shadow-[#0D99FF]/25"
        ...
        {tag && (
          <div className="mt-6 flex justify-center">
            <span className="inline-flex items-center gap-2 rounded-full border border-[#0D99FF]/30 bg-[#0D99FF]/10 py-1.5 pl-4 pr-2 text-sm font-semibold text-[#0D99FF]">
              Tag: {tag}
              <Link
                to={hrefWith({ tag: null, page: null })}
                preventScrollReset
        ...
        {totalPages > 1 && (
          ...
          >
            {page > 1 ? (
              <Link
                to={hrefWith({ page: page - 1 === 1 ? null : page - 1 })}
