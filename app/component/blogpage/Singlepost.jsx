...
const ShareLinks = ({ post, vertical = false }) => {
  const url =
...
const RelatedCard = ({ post }) => (
  <Link to={postPath(post)} className="group flex flex-col">
        ...
        <div className="relative mx-auto max-w-4xl px-4 pb-8 pt-10 sm:px-6 md:pt-14">
          ...
          <nav aria-label="Breadcrumb" className="text-sm text-[#4B5154]">
            <ol className="flex flex-wrap items-center gap-2">
              <li>
                <Link to="/" className="transition hover:text-[#0D99FF]">
                  Home
                </Link>
              </li>
              <li aria-hidden="true" className="text-gray-300">/</li>
              <li>
                <Link to="/blog" className="transition hover:text-[#0D99FF]">
                  Blog
                </Link>
              </li>
              <li aria-hidden="true" className="text-gray-300">/</li>
            ...
            {post.tags?.length > 0 && (
              <div className="mt-12 flex flex-wrap items-center gap-2 border-t border-gray-200 pt-8">
                ...
                {post.tags.map((tag) => (
                  <Link
                    key={tag.slug}
                    to={`/blog/?q=${encodeURIComponent(tag.name)}`}
