...
/** Normalized job — the shape the career page consumes. */
...
/** Loose shape of an easy.jobs record (any field may be missing). */
interface RawJob {
  id?: string | number;
  slug?: string;
  title?: string;
  is_remote?: boolean;
  remote_location_type?: { name?: string };
  job_address?: {
    city?: { name?: string };
    state?: { name?: string };
    country?: { name?: string };
  ...
  is_pinned?: boolean;
  is_expired?: boolean;
  show_on_career_page?: number;
  apply_url?: string;
  job_link?: string;
}

interface EasyJobsPayload {
  status?: string;
  data?: {
    message?: string;
    jobs?: { data?: RawJob[] };
...
let cache: { at: number; jobs: Job[] | null } = { at: 0, jobs: null };

/** Human-readable location: "Remote", or "City, Country". */
function formatLocation(raw: RawJob): string {
  if (raw.is_remote) {
 ...
 * are skipped.
 */
function normalizeList(list: RawJob[]): Job[] {
  const jobs: Job[] = [];
...
function fallback(reason: string): Job[] {
  if (cache.jobs) {
...
export async function getOpenJobs(): Promise<Job[]> {
  ...
  try {
    ...
    let payload: EasyJobsPayload;
    try {
      payload = (await res.json()) as EasyJobsPayload;
    } catch {
    ...
    const reason =
      err.name === "TimeoutError" || err.name === "AbortError"
        ...
        : `easy.jobs request failed: ${err.message}`;
